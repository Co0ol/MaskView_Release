...
public class mask_view extends PApplet {
...
public void mouseClicked(){
  ...
  if(p0 != null && p1 != null){
    if(mouseX >= width - p1.width){
      ...
      masks[count][6] = PApplet.parseInt(st);
      count++;
      count = constrain(count, 0, 19);
    }
  }
...
public void mask(){
  ...
  for(int j = 0; j < count; j++){
    ...
    drawMasks(mouseX, mouseY, maskWid, maskHei, maskAng, maskShape, PApplet.parseInt(st));

}
//posX, posY, maskWid, maskHei, maskAng, maskShape, stroke
public void drawMasks(int posX, int posY, int wid, int hei, int ang, int sha, int str){
  beginShape();
  if(str != 0) stroke(0);
  else noStroke();
...
class TF{
  ...
  public boolean showTF(boolean bo){
...
class slider{
  ...
  public float slide(){
    ...
    textSize(30);
    text(PApplet.parseInt(value), posX + 230, posY + 10);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "mask_view" };
    if (passedArgs != null) {
